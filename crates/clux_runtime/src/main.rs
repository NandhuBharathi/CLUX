...
fn main() {
    ...
    if args.len() > 1 && args[1] == "--info" {
        ...
        println!("  Stack Architecture       : 4-Layer SSM | SwiGLU | AdamW");
        println!("==================================================");
        return;
    }
    ...
    if args.len() > 2 && args[1] == "train" {
        ...
        let steps = args.get(3).and_then(|s| s.parse::<usize>().ok()).unwrap_or(1500);
    ...
    if args.len() > 2 && args[1] == "generate" {
        ...
        let prompt = &args[2];
        let max_tokens = args.get(3).and_then(|s| s.parse::<usize>().ok()).unwrap_or(35);
        let temp = args.get(4).and_then(|s| s.parse::<f32>().ok()).unwrap_or(0.6);
